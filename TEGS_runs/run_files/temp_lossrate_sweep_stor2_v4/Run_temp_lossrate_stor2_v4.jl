...
run_name = "temp_lossrate_sweep_stor2_v4"
...
lossrates = Array{Float64}([1, 2, 3, 4, 5]) #, 7, 8, 9, 10, 11, 12, 13, 14, 15])
temperatures = Array{Float64}([2400])
# lossrates = Array{Float64}([3])
dsratios = Array{Float64}([120, 100, 80, 60, 40])
    ...
    for (emiss_name, emiss_target) in emiss_targets[loc_name]
        ...
        for T in temperatures
            for lossrate in lossrates
                ...
                for dsratio in dsratios

                    # Calculate and save baseline emissions
                    outputs_path = joinpath(outputs_path_root, "$(emiss_name)_$(T)_$(lossrate)_stor2_ds$(dsratio)")
                    ...
                    @constraint(EP, TEGSfixDS, EP[:eTotalCapEnergy][end] == dsratio * EP[:eTotalCap][end])
                    # @constraint(EP, TEGSpower[t=1:myinputs["T"]], EP[:vP][end,t] * EP[:eTotalCapEnergy][end] <= EP[:vS][end,t] * EP[:eTotalCap][end])
                    @constraint(EP, TEGSpower[t=1:myinputs["T"]], EP[:vP][end,t] * dsratio <= EP[:vS][end,t])
                    # set_optimizer_attribute(EP, "NonConvex", 2)
