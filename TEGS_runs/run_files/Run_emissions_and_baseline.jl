...
############################################
root_dir = dirname(@__DIR__) # Should be ../TEGS_runs

location_dir = Dict{String, String}(
    "newEngland" => joinpath(root_dir, "data", "newEngland_baseline"),
    "texas" => joinpath(root_dir, "data", "texas_baseline"),
)

logging_notes = Array{String, 1}()

for (loc_name, loc_path) in location_dir
    run_name = "emissions_and_baseline_paperrev"
    ...
    mysetup = configure_settings(genx_settings, outputs_path) # mysetup dictionary stores settings and GenX-specific parameters
