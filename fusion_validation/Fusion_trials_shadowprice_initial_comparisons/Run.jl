    ...
    for therm_dis_ratio in therm_dis_ratio_list
        for turb_ratio in turb_ratio_list
            for reactor_cap in reactor_cap_list 
                ...
                outputs_path = joinpath(case, "Results", "ReactorCap_" * string(reactor_cap) * "_TurbRatio_" * string(turb_ratio) * "_ThermStor_" * string(therm_stor) * "_ThermDisRatio_" * string(therm_dis_ratio))
