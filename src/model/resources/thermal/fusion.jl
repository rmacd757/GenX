...
function fusionthermalpower(EP::Model, inputs::Dict, setup::Dict)
    ...
    NameplateCap = (dfGen[!,:Cap_Size])/0.4   ## Multiply by Heat Rate to convert electricity capacity to thermal power
...
function fusiongridpower(EP::Model, inputs::Dict, setup::Dict)
    ...
    @expression(EP, eTurbThermal[y in FUSION,t=1:T], vsaltpwr[y,t]*salteff/0.4 + EP[:vThermOutput][y,t] - saltLosses)
    ...
    @expression(EP, eFusionPower[y in FUSION,t=1:T], eTurbThermal[y,t]*0.4 - eRecircpwr[y,t])
