...
input_name = "2z_1sc_dual_basecase_SC"
case_name = "2z_1sc_dual_basecase_SC"
...
for year in task_id+1:num_tasks:length(year_list)
    ## Define input and output paths
    inputs_path = joinpath(@__DIR__, "year_$year")
    ...
    for idx in 1:length(all_cases)
        ...
        outputs_path = joinpath(results_path, "Dual_sc" * string(year) * "_" * string(fusion_cap) * "mw_EmissLevel_" * string(emiss_lim))
