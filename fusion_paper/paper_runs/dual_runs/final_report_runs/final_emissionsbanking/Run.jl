...
input_name = "updated_basecase_emissionsbanking"
case_name = "updated_basecase_emissionsbanking"
...
emission_banking_fraction = [0.10, 0.25, 0.50, 0.75]
...
all_cases = vcat(collect(Iterators.product(emiss_lim_list, fusion_cost_list, emission_banking_fraction))...)
...
for idx in task_id+1:num_tasks:length(all_cases)
    ...
    if !isfile(joinpath(outputs_path, "costs.csv"))
        println("Including Case for emiss limit = $emiss_lim, fusion cap = $fusion_cost, and emissions banking fraction = $emiss_bank_frac")
        push!(reduced_cases, (emiss_lim, fusion_cost, emiss_bank_frac))
...
for idx in task_id+1:num_tasks:length(all_cases)
    ...
    mysetup["CO2CapWiggle"] = emiss_bank_frac
