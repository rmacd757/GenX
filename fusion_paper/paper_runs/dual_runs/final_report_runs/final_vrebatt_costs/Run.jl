...
vrebatt_cost_mult_list = [0.5, 0.75, 1.25, 1.5]
...
all_cases = vcat(collect(Iterators.product(emiss_lim_list, fusion_cost_list, vrebatt_cost_mult_list))...)
...
for idx in task_id+1:num_tasks:length(all_cases)
    ...
    vrebatt_cost_mult = all_cases[idx][3]
    outputs_path = joinpath(results_path, "Cost_$(fusion_cost)_EmissLevel_$(emiss_lim)_gCO2perkWh_VREbattmult_$(vrebatt_cost_mult)")
...
for idx in task_id+1:num_tasks:length(all_cases)
    ...
    # Find all the RIDs for all the resources which have a name starting with the names in vre_stor_name_roots
    vre_stor_rids = Int64[]
    for name_root in vre_stor_name_roots
