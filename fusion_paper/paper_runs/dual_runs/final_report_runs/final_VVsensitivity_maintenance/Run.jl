...
fusion_cost_list = [6000.0, 8500.0]
...
all_cases = vcat(collect(Iterators.product(emiss_lim_list, fusion_cost_list, vessel_nameplate_lifetime_list, vessel_replacement_time_list))...)
...
for idx in task_id+1:num_tasks:length(all_cases)
    ...
    outputs_path = joinpath(results_path, "Cost_$(fusion_cost)_EmissLevel_$(emiss_lim)_gCO2perkWh_Nameplate_$(vessel_nameplate_lifetime)_Replacement_$(round(vessel_replacement_time,digits=3))")
...
for idx in task_id+1:num_tasks:length(all_cases)
    ...
    for y in fusion_rid
        ...
        dfFusion[y,:Vessel_Life] = vessel_nameplate_lifetime
        dfFusion[y,:Rep_Dur] = vessel_replacement_time
    end
